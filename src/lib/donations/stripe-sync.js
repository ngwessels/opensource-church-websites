...
function isDonationCheckoutSession(session) {
  ...
  return Boolean(metadata.fundId || metadata.fundLabel || metadata.frequency);
}

/**
 * Pull recent Stripe Checkout completions and subscription renewals into Firestore.
 * Safe to re-run (checkout docs overwrite; renewals skip duplicates).
 *
...
export async function syncDonationsFromStripe(db, stripe, options = {}) {
  ...
  /** @type {{ checkouts: { created: number, skipped: number, errors: string[] }, renewals: { created: number, skipped: number, errors: string[] } }} */
  const summary = {
