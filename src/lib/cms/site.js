...
export function getBuilderCapabilities() {
  return {
    ...
    mediaUpload: {
      ...
      workflow:
        "Local files: create_media_upload_link → open uploadUrl in a browser → choose file and click Upload → get_media_upload. Already-hosted files: upload_media / upload_media_batch with sourceUrl (server fetch, max 10MB). Do not send file bytes in MCP tool calls. Images go in pictures-root, PDFs in documents-root.",
    },
    ...
    playbooks: {
      fullParishSiteRedesign: [
        ...
        "Per page: create_media_upload_link (local files) or upload_media_batch (sourceUrl) → add_modules_batch → update_module for fine-tuning",
        "publish_all_pages",
      ],
      homepageFromScratch: [
        "update_page (home): layout, heroSlideshowEnabled: true, contentColumnsByViewport",
        "create_media_upload_link (or upload_media_batch with sourceUrl) for hero images",
      ...
      bulletinPosting: [
        ...
        "If the PDF is local: create_media_upload_link folderId=documents-root → open uploadUrl → get_media_upload",
        "If the PDF is already hosted: upload_media folderId=documents-root sourceUrl=...",
