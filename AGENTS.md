...
Config shape: `{ moduleInstanceId, title, description, submitLabel, notificationEmails }`. Fixed fields: intention (required); name, email, and phone are optional behind a closed accordion for anonymous submissions. Public success message is always `Thank you.` (not configurable) so moderation outcome is never revealed.
